...
# ---------------------------- Random Words ------------------------------- #
try:
    data = pds.read_csv("data\\words_to_learn_hindi.csv")
except FileNotFoundError:
    original_data = pds.read_csv("data\\hindi_words.csv")
...
def hindi_word_gen():
    ...
    canvas.itemconfig(word, text=current_card["Hindi"])
...
def remove_word():
    ...
    save.to_csv("data\\learning_list\\words_to_learn_hindi.csv", index=False)
    hindi_word_gen()
# ---------------------------- Counter ------------------------------- #
...
cross_button.config(border=0, background=DARK,
                    activebackground=DARK, command=hindi_word_gen)
